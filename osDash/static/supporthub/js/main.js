document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadFoundries() {
        ...
        if (!foundrySelect) return;

        try {
            const response = await fetch('/supporthub/api/foundries');
    ...
    async function updateFoundryInfo() {
        ...
        if (!foundryInput || !nameSelect || !emailSelect || !phoneSelect) return;

        const foundry = foundryInput.value;
